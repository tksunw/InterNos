...
**v2.1.0 (current, 2026-08-31): Electron AX-liar fix.** Chromium's AX layer accepts a `kAXSelectedTextAttribute` write into web content and reports success without inserting (field report: Claude Desktop, empty composer, transcript only in volatile recovery). Since AX success is trusted with no fallback, `TextInserter` now gates the AX path behind `axInsertionUnreliable` (default: `isElectronApp`, sniffs `Contents/Frameworks/Electron Framework.framework` in the target's bundle) — Electron apps go straight to clipboard swap.
