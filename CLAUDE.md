...
**v2.0.1 shipped 2026-08-29** (current): cleanup `validate()` strips the `<<<DICTATION`/`DICTATION>>>` prompt markers the model sometimes echoes back (macOS 26.6.1 field report). The markers live in `CleanupPrompt.openMarker`/`closeMarker`; any future output-hygiene fix belongs in `SmartCleanupCoordinator.validate()`, the single choke point.
