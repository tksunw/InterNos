...
**v2.0.0 shipped 2026-07-26** (merged to main; the first non-prerelease since 1.2.0, so `Check for Updates` offers it to 1.x users): command mode (second modifier key → AX selection capture → `CommandTransformer.swift` Foundation Models rewrite → replacement through the shared inserter), AX-direct insertion (`TextInserter.accessibilityInsert`, clipboard swap now fallback-only; AX success is trusted, volatile recovery is the net), live transcript preview (volatile results via `reportingOptions: [.volatileResults]` → `onPartial` → indicator), "scratch that" voice undo (backspace synthesis, one-shot, 2000-char cap), multi-language recognition (`recognitionLocale` default, engine reads it per call via a Sendable locale provider; commands stay English). **Cleanup ships Off by default in 2.0 — intentional, decided 2026-07-15** (beta testing kept finding new model-drift modes; verbatim-unless-asked is the trust posture; a default flip is a separate post-2.0 decision). Transcript history is a **permanent non-goal** (PRD).

**v2.0.1 shipped 2026-08-29**: cleanup `validate()` strips the `<<<DICTATION`/`DICTATION>>>` prompt markers the model sometimes echoes back (macOS 26.6.1 field report). The markers live in `CleanupPrompt.openMarker`/`closeMarker`; any future output-hygiene fix belongs in `SmartCleanupCoordinator.validate()`, the single choke point.

**v2.1.1 (current, 2026-09-01): code-review fixes.** The AX-liar gate broadened from Electron-only to all Chromium (finding: CEF apps like Spotify and Chromium browsers lie the same way — see the `isChromiumApp` marker sniff described under 2.1.0). Sparkle legacy migration now preserves an explicit opt-out (writes `automaticallyChecksForUpdates=false` instead of discarding the key); the Settings updates toggle re-reads Sparkle state on window revisit. Script hardening: debug builds strip `SUFeedURL` (dev app must never be a live Sparkle client — TCC), `release.sh` refuses to build unless `CFBundleVersion` exceeds the newest appcast `sparkle:version`, `sign-sparkle-framework.sh` enumerates nested executables dynamically. Known deferred item: no `SPUStandardUserDriverDelegate` gentle reminders, so a scheduled-check alert can steal focus mid-dictation (rare; automatic checks are opt-in) — implement before automatic checks become common.

**v2.1.0 (2026-08-31): Electron AX-liar fix.** Chromium's AX layer accepts a `kAXSelectedTextAttribute` write into web content and reports success without inserting (field report: Claude Desktop, empty composer, transcript only in volatile recovery). Since AX success is trusted with no fallback, `TextInserter` now gates the AX path behind `axInsertionUnreliable` (default: `isChromiumApp`, cached per bundle path; sniffs `Contents/Frameworks` for the Electron/CEF frameworks or a Chromium browser's `* Framework.framework` carrying `libvk_swiftshader.dylib`/`libEGL.dylib`) — Chromium-based apps go straight to clipboard swap.

**v2.1.0: Sparkle in-place updates** (replaces the hand-rolled `UpdateChecker`, deleted). `UpdateController.swift` wraps `SPUStandardUpdaterController`; automatic checks stay opt-in (Sparkle's one-time prompt or the Settings toggle; a legacy `checkUpdatesAtLaunch` opt-in migrates once, then the key is removed). Feed: `appcast.xml` at repo root, served via raw.githubusercontent.com from `main` (`SUFeedURL`); EdDSA public key in `Info.plist` (`SUPublicEDKey`), private key in the login keychain (created via Sparkle's `generate_keys`). `make-app.sh` embeds `Sparkle.framework` from the SPM artifact into `Contents/Frameworks` and re-signs its nested executables via `scripts/sign-sparkle-framework.sh` (library validation demands our team's signature); `release.sh` re-signs with Developer ID + `--timestamp` and runs `generate_appcast` (release flow: upload DMG to the GitHub release, then commit + push `appcast.xml` — updaters can't see the version before that). 1.x/2.0.x users still update through the old GitHub-API path once more; Sparkle takes over from the next release.
